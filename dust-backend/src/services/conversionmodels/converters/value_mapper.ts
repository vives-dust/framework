import { ConversionSample } from '../../../typebox-types/conversion_sample';
...
export class ValueMapper extends Converter {
  ...
  convert_value(input: Measurement): Measurement {
  ...
  convert_values(input: Measurement[]): Measurement[] {
    const output = input.map(s => s = this.convert_value(s));
    return output;
  }
